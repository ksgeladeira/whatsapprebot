...
const ytshit = require('.\\yt.js');
const errorurl = 'https://steamuserimages-a.akamaihd.net/ugc/954087817129084207/5B7E46EE484181A676C02DFCAD48ECB1C74BC423/?imw=512&&ima=fit&impolicy=Letterbox&imcolor=%23000000&letterbox=false'
const errorurl2 = 'https://steamuserimages-a.akamaihd.net/ugc/954087817129084207/5B7E46EE484181A676C02DFCAD48ECB1C74BC423/?imw=512&&ima=fit&impolicy=Letterbox&imcolor=%23000000&letterbox=false'
const unlistedCMD = require('./commands/notlistedcommands.js');
...
🌐️ *URL*: ${data.url}`
        ...
        if (!isBanned) {
            ...
            case 'stiker':
                if (isMedia) {
                    ...
                    if(isGroupMsg) {
                        client.sendTextWithMentions(from, `@${message.author} Voce não marcou a foto`)
                    } else {
                        client.reply(from, 'Menciona uma foto', message)
                    }
                }
            break
        case 'tsticker':
        ...
        case 'sgif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 12 || mimetype === 'image/gif' && message.duration < 12) {
                    const mediaData = await decryptMedia(message, uaOverride)
                    ...
                    client.reply(from, '[❗] Envie um vídeo com uma legenda *!stickerGif* Max 12 segundos exato!', id)
                )
            }
            break
        case 'stickernobg':
...
${desc}`)
       ...
       case 'cgc':
             ...
             const {
                ...
                if (nsfw == true) {
                      if ((isGroupMsg) && (isnsfw)) {
                                ...
                                await client.reply(from, `NSFW Não está registrado *${name}*`, id)
                      }
                } else { 
                      await client.sendFileFromUrl(from, `${url}`, 'Reddit.jpg', `${title}` + '\n\nPostlink:' + `${postLink}`)
                }
                    ...
                    console.log(err)
                    await client.reply(from, 'Isso não é um subreddit!', id) 
                }
                break
        case 'unban':
            if(!isowner) return client.reply(from, 'Only bot admins can use this CMD', message.id)
        ...
        case 'kikar':
            ...
            if(!isBotGroupAdmins) return client.reply(from, 'O Bot precisa ser adm', message.id)
            if(mentionedJidList.length === 0) return client.reply(from, 'Wrong format', message.id)
            await client.sendText(from, `Banido com sucesso! issued:\n${mentionedJidList.join('\n')}`)
            for (let i = 0; i < mentionedJidList.length; i++) {
            ...
            break
        case 'delete':
            if (!isGroupAdmins) return client.reply(from, 'Somente Adms podem utilizar o comando', id)
        ...
        case 'lyrics':
            ...
            break
        case 'wallpaper':
            if (args.length == 0) return client.reply(from, 'Formato Incorreto', id)
        ...
        case 'mp3':
            ...
            YD.on("finished", function(err, data) {
                client.sendPtt(from, `./media/mp3/${data.videoTitle}.mp3`, id)
                //console.log(JSON.stringify(data));
            });
             
            YD.on("error", function(error) {
                client.reply(from, `Sorry ${pushname}, An Error Occured!`, message.id);
                console.log(error);
            });
             
            YD.on("progress", function(progress) {
                client.reply(from, `Converting, Please Wait Senpai uwu)/`, message.id);
